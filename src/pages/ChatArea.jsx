...
import { doc, updateDoc, arrayUnion, Timestamp, setDoc, getDoc, onSnapshot, increment, query, collection, where } from "firebase/firestore";
...
const ChatArea = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const chatPath = isGlobal ? doc(db, "groups", "global-chat") : doc(db, "chats", chatId)
      ...
      setText("");
      setImg(null);
    } catch (error) { console.log("Error in handleSend:", error); }
  };
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snapShot) => {
      ...
      snapShot.forEach(doc => {
        const userData = doc.data()
        if(userData?.uid !== currentUser?.uid) users.push({ ...doc.data() , id: doc.id })   
      });
...
return (
  <div className={`bg-[#F4F6FA] dark:bg-[#20232D] md:w-[70%] w-full h-screen flex flex-col overflow-hidden ${user ? "flex" : "hidden"} md:flex transition-colors duration-300`}>
    {user ? (
      <>
        <div className="flex-none w-full h-[70px] border-b border-gray-300 dark:border-gray-700 bg-[#F4F6FA] dark:bg-[#20232D] flex items-center justify-between px-4 z-50">
          <div className="flex items-center gap-3">
             <div onClick={() => dispatch(changeUser({ user: "" , chatId: ""})) } className="block lg:hidden cursor-pointer">
                <FaAngleLeft className="text-black dark:text-white" size={24} />
             </div>
             {user.uid !== "global-chat" && (
                <img src={user.photoURL} className="rounded-full h-[45px] w-[45px] object-cover" />
             )}
             <div className="flex flex-col">
                <span className="text-black dark:text-white font-bold text-lg">{user?.displayName}</span>
             </div>
          </div>
          
          <div className="flex flex-col items-center">
            <div className="flex -space-x-2">
            {user?.uid === "global-chat" && onlineUsers.slice(0, 4).map((item) => (
              
              <img key={item.uid} src={item.photoURL} className="h-9 w-9 rounded-full border-2 border-white dark:border-[#20232D]" />
            ))}
            {remainingCount > 0 && (
        ...
        <div className="flex-none p-4 bg-transparent">
          <div className="bg-white dark:bg-[#1A1C20] shadow-sm rounded-full lg:rounded-lg w-full h-[55px] flex items-center px-4 gap-3 border dark:border-gray-800">
            ...
            <div className="flex items-center gap-3 text-gray-400">
              ...
              <label htmlFor="file"><CiImageOn className={`cursor-pointer ${img ? "text-blue-500" : ""}`} size={24} /></label>
              <BsPaperclip className="cursor-pointer" size={20} />
              <button onClick={handleSend} className={`${text.length > 0 ? "bg-[#4183D8] border-[#4183D8] text-white" : "border-gray-400" } dark:text-white border border-gray-400 p-2 rounded-full hover:scale-105 transition-all`}>
                <MdSend size={20} />
              </button>
            </div>
          </div>
      ...
      <div className="flex flex-col flex-1 justify-center items-center opacity-50">
        ...
        <p className="mt-4 font-medium dark:text-white text-black">Select a friend to start chatting</p>
      </div>
    )}
  </div>
);
